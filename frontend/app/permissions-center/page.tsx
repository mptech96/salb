...
import { DataTableShell, EmptyState, FilterBar, FormSection, PageHeader, StatusBadge, fieldClassName, primaryButtonClassName, secondaryButtonClassName } from "@/components/ui/enterprise";
...
export default function PermissionsCenter() {
  ...
  return <section dir="rtl" className="space-y-5">
    ...
    <FormSection title="المستخدم والصلاحيات"><FilterBar><div className="min-w-0 flex-1"><SearchSelect value={userId} onChange={value=>void pick(value)} placeholder="اختر المستخدم" searchPlaceholder="ابحث باسم المستخدم، الدور أو الفرع..." options={users.map(user=>({value:user.id,label:`${user.name} — ${user.role_name||user.role_code||"بدون دور"} — ${user.branch_name||"الشركة"}`,search:`${user.name||""} ${user.role_name||user.role_code||""} ${user.branch_name||""}`}))}/></div><input className={`${fieldClassName} sm:max-w-xs`} value={q} onChange={event=>setQ(event.target.value)} placeholder="بحث في الإجراءات والصلاحيات"/></FilterBar>{detail&&<div className="mt-3 flex flex-wrap gap-2"><StatusBadge tone="info">الدور: {detail.role?.role_name||detail.role?.role_code||"—"}</StatusBadge><StatusBadge tone="success">فعّالة: {effective.size}</StatusBadge><StatusBadge>موروثة من الدور: {inherited.size}</StatusBadge></div>}</FormSection>
