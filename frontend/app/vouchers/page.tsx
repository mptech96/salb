...
export default function VouchersPage() {
  ...
  const message = (type: string, title: string, msg: string) =>
    setDialog({ open: true, type, title, message: msg });
  const apiMsg = (e: any) =>
  ...
  async function load(next = filters) {
    ...
    try {
      ...
      message("error", "تعذر تحميل السندات", apiMsg(e));
    } finally {
      setLoading(false);
    }
  }
  ...
  const references = useMemo(() => {
    const filter = (a: any[]) =>
      a.filter((x: any) => {
        if (Number(x.scope_all_branches) === 1) return true;
        if (x.branch_ids) return x.branch_ids.map(String).includes(branch);
        return !x.branch_id || String(x.branch_id) === branch;
      });
    if (form.reference_type === "SUPPLIER")
      return filter(meta.suppliers || []).map((x: any) => ({
        id: x.id,
        name: x.supplier_name,
      }));
    if (form.reference_type === "DRIVER")
      return filter(meta.drivers || []).map((x: any) => ({
        id: x.id,
        name: x.driver_name,
      }));
    if (form.reference_type === "WORKER")
      return filter(meta.workers || []).map((x: any) => ({
        id: x.id,
        name: x.worker_name,
      }));
    return filter(meta.customers || []).map((x: any) => ({
      id: x.id,
      name: x.customer_name,
    }));
  }, [form.reference_type, branch, meta]);
  const chosenFa = fa.find(
    (x: any) => String(x.id) === String(form.financial_account_id),
  );
  const docAmount = n(form.amount);
  const baseAmount = docAmount * n(form.exchange_rate || 1);
  function openNew() {
    const b = String(meta.scoped_branch_id || meta.branches?.[0]?.id || "");
    setForm({
      ...blank(),
      branch_id: b,
      currency_code:
  ...
  function pickMethod(v: string) {
    const wanted = ["BANK", "BANK_TRANSFER", "CARD"].includes(v)
      ...
      : v === "WALLET"
        ? "WALLET"
        : "CASH";
    const x = fa.find((a: any) => a.account_type === wanted) || fa[0];
    setForm({
      ...form,
      payment_method: v,
  ...
  async function save() {
    if (
      ...
      return message(
        "warning",
        "بيانات ناقصة",
        "حدد الفرع والجهة والتاريخ والمبلغ.",
      );
    setSaving(true);
    try {
      await api.post("/vouchers", {
        ...
        amount: baseAmount,
        foreign_amount:
          form.currency_code !== meta.base_currency ? docAmount : null,
        exchange_rate: n(form.exchange_rate) || 1,
      });
      message(
        "success",
        "تم ترحيل السند",
        `المبلغ الدفتري ${money(baseAmount)} ${meta.base_currency || ""} من خلال ${chosenFa?.account_name || "الحساب المالي الافتراضي"}.`,
      );
      setShow(false);
      await load();
    } catch (e: any) {
      message("error", "تعذر حفظ السند", apiMsg(e));
    } finally {
      setSaving(false);
    }
  }
  const refName = (x: any) => {
    ...
    const p = pools.find((z: any) => String(z.id) === String(x.reference_id));
    return (
      p?.customer_name ||
      p?.supplier_name ||
      p?.driver_name ||
  ...
  return (
    <section dir="rtl" className="space-y-5">
      <Header
        ...
        subtitle="كل حركة مرتبطة بفرع وخزينة/بنك فعلي، بينما يبقى حساب الأستاذ موحدًا للشركة."
        onAdd={openNew}
      />
      <div className="grid gap-3 sm:grid-cols-3">
        <Card t="نتائج الفلتر" v={pagination.total} />
        <Card t="إجمالي القبض" v={money(summary.receipt_total)} />
        <Card t="إجمالي الصرف" v={money(summary.payment_total)} />
      </div>
      <div className="grid gap-3 rounded-2xl border bg-white p-4 md:grid-cols-3 xl:grid-cols-7">
        <input
        ...
        <select
          ...
          onChange={(e) =>
            setFilters({ ...filters, voucher_type_id: e.target.value, page: 1 })
          }
        >
          <option value="">قبض وصرف</option>
          {meta.voucher_types?.map((x: any) => (
              ...
              {!rows.length ? (
                <tr>
                  <td colSpan={9} className="p-8 text-center text-slate-500">
                    {loading ? "جاري التحميل..." : "لا توجد سندات"}
                  </td>
                </tr>
              ) : (
                rows.map((x: any) => (
                  <tr key={x.id} className="border-t">
                    ...
                    <td className="p-4">{x.voucher_date}</td>
                    <td className="p-4">{x.branch_name || "-"}</td>
                    <td className="p-4">{x.type_name || x.voucher_type_id}</td>
                    <td className="p-4">{refName(x)}</td>
      ...
      {show && (
        <Modal title="سند مالي جديد" close={() => setShow(false)}>
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <Select
              l="نوع السند"
              v={form.voucher_type_id}
              set={(v) => setForm({ ...form, voucher_type_id: v })}
              o={(meta.voucher_types || []).map((x: any) => ({
                id: x.id,
                name: x.type_name,
              }))}
            />
            <Input
              ...
              type="date"
              v={form.voucher_date}
              set={(v) => setForm({ ...form, voucher_date: v })}
            />
            <Select
              l="نوع الجهة"
              v={form.reference_type}
              set={(v) =>
                setForm({ ...form, reference_type: v, reference_id: "" })
              }
              o={[
                { id: "CUSTOMER", name: "عميل" },
                { id: "SUPPLIER", name: "مورد" },
                { id: "DRIVER", name: "سائق" },
                { id: "WORKER", name: "عامل/موظف" },
              ]}
            />
            <Select
              l="الجهة *"
            ...
            <Input
              l="رقم السند - اختياري"
              v={form.voucher_number}
              set={(v) => setForm({ ...form, voucher_number: v })}
            />
            <Select
              l="طريقة الدفع"
            ...
            <Select
              ...
              set={(v) => {
                const x = fa.find((a: any) => String(a.id) === v);
                setForm({
                  ...form,
            ...
            <div className="rounded-2xl bg-slate-50 p-4">
              <div className="text-xs text-slate-500">المبلغ الدفتري</div>
              <div className="text-xl font-black">
                {money(baseAmount)} {meta.base_currency}
              </div>
            </div>
          </div>
          <textarea
          ...
          >
            {saving ? "جاري الترحيل..." : "حفظ وترحيل السند"}
          </button>
        </Modal>
      )}
      <SystemDialog
...
function Header({ title, subtitle, onAdd }: any) {
  return (
