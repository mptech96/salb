...
export default function PrintHeader({ profile, title, filters, family="report", showApprovals=true }: Props) {
  ...
  const companyFields = options.company_fields || {};
  const fieldVisible = (field: string) => companyFields[field] !== false;
    ...
    <header dir={localeDirection(locale)} className={`print-branding-header relative z-10 border-b-2 ${options.variant === "COMPACT" ? "mb-2 pb-2" : "mb-5 pb-4"}`} style={{borderColor:options.variant === "MODERN" ? profile?.secondary_color || "#123D68" : profile?.primary_color||"#0B2A4A"}}>
      ...
      <div className="flex items-start justify-between gap-5">
        <div className="flex items-start gap-4">
          {printVisible(options,"logo") && options.header_mode !== "TEXT" && <BrandingAssetImage asset="logo" scopeKey={profile?.company_id} enabled={Boolean(profile?.logo_url || profile?.logo_data_uri || profile?.has_logo || profile?.logo_path)} alt="شعار الشركة" className="object-contain" style={{width:`${logoWidth}mm`,maxHeight:`${Math.max(12, Math.min(45, logoWidth))}mm`}} />}
          <div>
            ...
            {!printVisible(options,"company_details") ? null : <><div className="mt-1 text-sm text-slate-500">
              {[
                fieldVisible("phone") ? profile?.phone || profile?.print_phone : null,
                fieldVisible("email") ? profile?.email || profile?.print_email : null,
                fieldVisible("city") ? profile?.city || profile?.print_city : null,
              ].filter(Boolean).join(" • ")}
            </div>
            {fieldVisible("address") && (profile?.address || profile?.print_address) ? <div className="text-xs text-slate-500">{profile.address || profile.print_address}</div> : null}</>}
        ...
        <div className="text-left text-xs text-slate-500">
          {printVisible(options,"commercial_register") && fieldVisible("commercial_register") && profile?.commercial_register ? <div>السجل: {profile.commercial_register}</div> : null}
          {printVisible(options,"tax_number") && fieldVisible("tax_number") && profile?.tax_number ? <div>الرقم الضريبي: {profile.tax_number}</div> : null}
        </div>
      </div></>}
