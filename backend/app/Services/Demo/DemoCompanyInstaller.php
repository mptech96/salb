...
final class DemoCompanyInstaller
{
    ...
    public function install(string $password, ?string $resetPassword=null): array
    {
        ...
        return DB::transaction(function()use($password,$plan){
            ...
            [$customers,$suppliers]=$this->parties($cid,$bid);$items=$this->items($cid);$accounts=$this->financialAccounts($cid,$bid);$expenseTypes=$this->expenseTypes->ensure($cid);
            ...
            $this->expenseActivity($cid,$bid,$uid,$accounts,$expenseTypes,$today);
    ...
    private function expenseActivity(int$cid,int$bid,int$uid,array$accounts,array$types,CarbonImmutable$today): void
    {if(!$types)throw new RuntimeException('No company-specific Demo expense types are available.');for($i=0;$i<10;$i++){$date=$this->demoDate($today,45+$i);$type=DB::table('expense_types')->where('id',$types[$i%count($types)])->where('company_id',$cid)->where('is_active',1)->first();if(!$type||!$type->account_id)throw new RuntimeException('A Demo expense type is not valid for the Demo company.');$id=DB::table('expenses')->insertGetId(['company_id'=>$cid,'branch_id'=>$bid,'expense_type_id'=>$type->id,'expense_date'=>$date,'scope_type'=>'GENERAL','amount'=>100+$i*25,'payment_status'=>'PAID','payment_method'=>'CASH','financial_account_id'=>$accounts[$i%count($accounts)],'currency_code'=>'SAR','exchange_rate'=>1,'expense_effect'=>'COST','notes'=>'مصروف عرض تجريبي','created_at'=>now(),'updated_at'=>now()]);$result=$this->expenses->post(['company_id'=>$cid,'branch_id'=>$bid,'expense_id'=>$id,'amount'=>100+$i*25,'expense_date'=>$date,'payment_status'=>'PAID','payment_method'=>'CASH','financial_account_id'=>$accounts[$i%count($accounts)],'currency_code'=>'SAR','exchange_rate'=>1,'foreign_amount'=>100+$i*25,'expense_account_id'=>$type->account_id,'created_by'=>$uid]);if(!$result->success)throw new RuntimeException($result->message);}}
