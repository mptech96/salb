...
final class DemoIntegrityService
{
    ...
    public function validate(int $companyId, int $financialYearId): array
    {
        ...
        $unbalanced=$this->countUnbalancedPostedJournals($companyId);
        $duplicates=$this->countDuplicateSourceJournals($companyId);
    ...
    public function countUnbalancedPostedJournals(int $companyId): int
    {
        return DB::table('journal_entries as e')
            ->join('journal_entry_lines as l','l.journal_entry_id','=','e.id')
            ->where('e.company_id',$companyId)
            ->where('e.status','POSTED')
            ->selectRaw('e.id AS journal_entry_id, COALESCE(SUM(l.debit),0) AS total_debit, COALESCE(SUM(l.credit),0) AS total_credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit),0) AS difference')
            ->groupBy('e.id')
            ->havingRaw('ABS(COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit),0)) > 0.0001')
            ->get()
            ->count();
    }

    public function countDuplicateSourceJournals(int $companyId): int
    {
        return DB::table('journal_entries')
            ...
            ->selectRaw('source_type, source_id, COUNT(*) AS duplicate_count')
