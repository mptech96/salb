...
class ReportCenterService
{
    ...
    private function partyBalances(int $cid, ?int $bid, string $type, array $f): array
    {
        ...
        $m=$map[$type];$q=DB::table($m['table'])->where('company_id',$cid);if($bid!==null)$q->where(function($x)use($bid){$x->whereNull('branch_id')->orWhere('branch_id',$bid);});$this->search($q,$f,[$m['name'],'phone']);$entities=$q->select('id',$m['name'],'phone',DB::raw($type==='DRIVER'?'0 opening_balance':'COALESCE(opening_balance,0) opening_balance'))->orderBy($m['name'])->get();
