...
class InventoryLotService
{
    ...
    public function consumeFifo(int $cid,int $bid,int $itemId,float $qtyKg,string $sourceType,int $sourceId,?int $carId=null,?int $userId=null): array
    {
        ...
        foreach($lots as $lot){if($remaining<=0)break;$take=min($remaining,(float)$lot->qty_remaining_kg);$unit=round((float)$lot->unit_cost_per_kg,6);$cost=round($take*$unit,3);$newRemain=round((float)$lot->qty_remaining_kg-$take,3);$newSold=strtoupper($sourceType)==='SALE'?round((float)$lot->qty_sold_kg+$take,3):round((float)$lot->qty_sold_kg,3);
