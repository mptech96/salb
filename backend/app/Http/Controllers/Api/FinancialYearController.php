...
class FinancialYearController extends Controller
{
 ...
 public function close(Request $r,int $id,FinancialYearService $s,AccountingContext $c){$v=$r->validate(['confirm_year_close'=>'required|accepted']);try{return response()->json(['status'=>true,'message'=>'تم إقفال السنة وفتح السنة التالية تلقائيًا.','data'=>$s->close($c->companyId($r),$id,$c->userId($r),(bool)$v['confirm_year_close'])]);}catch(\Throwable $e){return response()->json(['status'=>false,'message'=>$e->getMessage()],422);}}
