...
class CarController extends Controller
{
    public function meta(Request $r,AccountingContext $ctx)
    {
        ...
        return response()->json(['status'=>true,'data'=>[
            ...
            'drivers'=>DB::table('drivers')->where('company_id',$cid)->where('is_active',1)->when($scoped!==null,fn($q)=>$q->where(fn($x)=>$x->where('branch_id',$scoped)->orWhereNull('branch_id')))->orderBy('driver_name')->get(['id','driver_name']),
    ...
    public function destroy(Request $r,int $id,AccountingContext $ctx){$cid=$ctx->companyId($r);$bid=$ctx->branchFilter($r);$q=DB::table('cars')->where('company_id',$cid)->where('id',$id);if($bid!==null)$q->where('branch_id',$bid);$car=$q->first();if(!$car)return response()->json(['status'=>false,'message'=>'السيارة غير موجودة ضمن نطاق فرعك.'],404);if(DB::table('weighbridge_cards')->where('company_id',$cid)->where('car_id',$id)->exists()||DB::table('shipments')->where('company_id',$cid)->where('car_id',$id)->exists()){DB::table('cars')->where('company_id',$cid)->where('id',$id)->update(['is_active'=>0,'updated_at'=>now()]);return response()->json(['status'=>true,'message'=>'للسيارة سجل تشغيلي؛ تم تعطيلها بدل حذفها.']);}DB::table('cars')->where('company_id',$cid)->where('id',$id)->delete();return response()->json(['status'=>true,'message'=>'تم حذف السيارة غير المستخدمة.']);}
    ...
    private function save(Request $r,AccountingContext $ctx,?int $id)
    {
        ...
        try{
            ...
            if(!empty($v['driver_id'])&&!DB::table('drivers')->where('company_id',$cid)->where('id',$v['driver_id'])->where('is_active',1)->when($scoped!==null,fn($q)=>$q->where(fn($x)=>$x->where('branch_id',$scoped)->orWhereNull('branch_id')))->exists())throw new \RuntimeException('السائق الافتراضي غير صالح ضمن نطاق الفرع.');
            ...
            if($id){$target=DB::table('cars')->where('company_id',$cid)->where('id',$id);if($scoped!==null)$target->where('branch_id',$scoped);if(!$target->exists())return response()->json(['status'=>false,'message'=>'السيارة غير موجودة ضمن نطاق فرعك.'],404);DB::table('cars')->where('company_id',$cid)->where('id',$id)->update($data);return response()->json(['status'=>true,'message'=>'تم تحديث السيارة.']);}
