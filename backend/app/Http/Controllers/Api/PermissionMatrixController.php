...
class PermissionMatrixController extends Controller
{
    ...
    public function index(Request $r,AccountingContext $ctx,SessionContextService $sessions)
    {
        ...
        $permissions=DB::table('permissions')->select('id','permission_name','permission_code','module_name')->orderBy('module_name')->orderBy('permission_name')->get();
    ...
    public function show(Request $r,int $userId,AccountingContext $ctx,SessionContextService $sessions)
    {
        ...
        $role=$sessions->roleForUser($userId);$base=$this->basePermissions($userId,$cid);$over=DB::table('user_permission_overrides as o')->join('permissions as p','p.id','=','o.permission_id')->where('o.company_id',$cid)->where('o.user_id',$userId)->get(['p.permission_code','o.effect']);
